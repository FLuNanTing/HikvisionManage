...
public interface HCNetSDK extends StdCallLibrary {
	...
	public static String filePath() {
		String file = HCNetSDK.class.getResource("/").getPath() + "static/HCNetSDK";
		file = file.substring(1, file.length());
		file = file.replace("/", "\\");
		LoggerUtil.info("HCNetSDK地址为:" + file);
		LoggerUtil.info("library.path: " + System.getProperty("java.library.path"));
		return file;
	}

	HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(filePath(), HCNetSDK.class);
	...
	public static final int MAX_LICENSE_LEN = 24;
	...
	public static class NET_DVR_IPALARMOUTCFG
			extends
				Structure {/* IP报警输出配置结构 */
		public int dwSize; /* 结构大小 */
	...
	public static class NET_DVR_DECCHANSTATUS
			extends
				Structure {/* 当前设备解码连接状态 */
	...
	public static class NET_DVR_SETUPALARM_PARAM extends Structure {
		...
		public byte byAlarmInfoType;// 智能交通报警信息上传类型：0-
									// 老报警信息（NET_DVR_PLATE_RESULT），1-
									// 新报警信息(NET_ITS_PLATE_RESULT)
		public byte byRetAlarmTypeV40;// 0-
										// 移动侦测、视频丢失、遮挡、IO信号量等报警信息以普通方式上传（报警类型：COMM_ALARM_V30，报警信息结构体：NET_DVR_ALARMINFO_V30），1-
										// 报警信息以数据可变长方式上传（报警类型：COMM_ALARM_V40，报警信息结构体：NET_DVR_ALARMINFO_V40，设备若不支持则仍以普通方式上传）
		public byte byRetDevInfoVersion;// CVR上传报警信息类型(仅对接CVR时有效)：0-
										// COMM_ALARM_DEVICE（对应报警信息结构体：NET_DVR_ALARMINFO_DEV），1-
										// COMM_ALARM_DEVICE_V40（对应报警信息结构体：NET_DVR_ALARMINFO_DEV_V40）
		public byte byRetVQDAlarmType;// VQD报警上传类型(仅对接VQD诊断功能的设备有效)：0-
										// COMM_ALARM_VQD（对应报警信息结构体：NET_DVR_VQD_DIAGNOSE_INFO），1-
										// COMM_ALARM_VQD_EX（对应报警信息结构体：NET_DVR_VQD_ALARM，包含前端设备信息和抓拍图片）
		public byte byFaceAlarmDetection;// 人脸报警信息类型：1-
											// 人脸侦测报警(报警类型：COMM_ALARM_FACE_DETECTION，NET_DVR_FACE_DETECTION)，0-
											// 人脸抓拍报警(报警类型：COMM_UPLOAD_FACESNAP_RESULT，NET_VCA_FACESNAP_RESULT)
		public byte bySupport;// 按位表示，每一位取值表示不同的能力 bit0-
								// 表示二级布防是否上传图片，值：0-上传，1-不上传 Bit1-
								// 表示是否启用断网续传数据确认机制，值：0-不开启，1-开启
	...
	public static class NET_DVR_PLATE_RESULT extends Structure {
		...
		public NET_DVR_VEHICLE_INFO struVehicleInfo;// 车辆信息参数
	...
	public static class NET_ITS_PICTURE_INFO extends Structure {
		...
		public byte byType;// 数据类型：0-车牌图，1- 场景图，2- 合成图，3- 特写图，4- 二值图，5- 码流，6-
							// 人脸子图(主驾驶)，7- 人脸子图(副驾驶)，8- 非机动车，9- 行人，10-
							// 称重原始裸数据，11- 目标图，12- 主驾驶室图，13- 副驾驶室图，14- 人脸图抠小图
		public byte byDataType;// 数据上传方式：0-数据直接上传;
	...
	public static class NET_ITS_PLATE_RESULT extends Structure {
		public int dwSize;// 结构体大小
		public int dwMatchNo;// 匹配号码
		public byte byGroupNum;// 图片组数量
		public byte byPicNo;// 连拍的图片数量
		public byte bySecondCam;// 是否第二相机抓拍
		public byte byFeaturePicNo;// 闯红灯电警，取第几张图作为特写图，0xff-表示不取
		public byte byDriveChan;// 触发车道号
		public byte byVehicleType;// 车型识别
		public byte byDetSceneID;// 检测场景号，0表示无效，其他取值：[1,4]，IPC为0（不支持）
	...
	boolean NET_DVR_GetDVRIPByResolveSvr_EX(String sServerIP, short wServerPort, String sDVRName, short wDVRNameLen, String sDVRSerialNumber, short wDVRSerialLen, String sGetIP,
